...
class MitraApi {
  ...
  /// Performs an HTTP 'POST /Mitra' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [PostCreateMitraDTO] postCreateMitraDTO:
  Future<Response> mitraPostWithHttpInfo({ PostCreateMitraDTO? postCreateMitraDTO, }) async {
    // ignore: prefer_const_declarations
    final path = r'/Mitra';

    // ignore: prefer_final_locals
    Object? postBody = postCreateMitraDTO;
  ...
  ///
  /// * [PostCreateMitraDTO] postCreateMitraDTO:
  Future<Mitra?> mitraPost({ PostCreateMitraDTO? postCreateMitraDTO, }) async {
    final response = await mitraPostWithHttpInfo( postCreateMitraDTO: postCreateMitraDTO, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
