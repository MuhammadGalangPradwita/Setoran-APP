...
class MotorImageApi {
  ...
  /// Performs an HTTP 'PUT /api/MotorImage' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [PutMotorImageDTO] putMotorImageDTO:
  Future<Response> apiMotorImagePutWithHttpInfo({ PutMotorImageDTO? putMotorImageDTO, }) async {
    // ignore: prefer_const_declarations
    final path = r'/api/MotorImage';

    // ignore: prefer_final_locals
    Object? postBody = putMotorImageDTO;
  ...
  ///
  /// * [PutMotorImageDTO] putMotorImageDTO:
  Future<MotorImage?> apiMotorImagePut({ PutMotorImageDTO? putMotorImageDTO, }) async {
    final response = await apiMotorImagePutWithHttpInfo( putMotorImageDTO: putMotorImageDTO, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
