...
class GetNotifikasDTO {
  /// Returns a new [GetNotifikasDTO] instance.
  GetNotifikasDTO({
    this.idNotifikasi,
    this.idPengguna,
    this.judul,
  ...
  /// Returns a new [GetNotifikasDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GetNotifikasDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GetNotifikasDTO[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GetNotifikasDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return GetNotifikasDTO(
  ...
  static List<GetNotifikasDTO> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GetNotifikasDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GetNotifikasDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GetNotifikasDTO> mapFromJson(dynamic json) {
    final map = <String, GetNotifikasDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GetNotifikasDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GetNotifikasDTO-objects as value to a dart map
  static Map<String, List<GetNotifikasDTO>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GetNotifikasDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = GetNotifikasDTO.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
